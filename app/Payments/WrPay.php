...
public function pay($order)
{
    ...
    // 1. 组装参数（严格对照文档）
    $params = [
        'mchId' => $this->config['pid'],
        'wayCode' => $this->config['type'], // 必须有值
        ...
        //'returnUrl' => 'https://www.google.com.hk', // 可选
    ...
    $signStr .= '&key='.$this->config['key']; // 商户密钥
...
// 4. 发起请求（POST JSON）
    ...
    file_put_contents(__DIR__ . '/Wrpay_debug.log', $log, FILE_APPEND);  
    */
    // 7. 返回 pay_url（或保持旧风格）
    return [
...
// 或者 strtoupper(md5($signStr));
    ...
    /*            // 写日志方便排查
        file_put_contents(__DIR__ . '/notify_debug.log',
            ...
            */
    return [
        'trade_no'    => $params['outTradeNo'],  // 本地订单号
        'callback_no' => $params['tradeNo'],  // 平台回调订单号（如果平台有单独的 serialOrderNo，这里最好用那个）
    ];
    }
